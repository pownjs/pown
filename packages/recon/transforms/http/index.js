...
const httpFingerprint = class extends Transform {
  ...
  async handle(
    { id: source = '', label = '', props, ...rest },
    {
      timeout = DEFAULT_TIMEOUT,
      follow = DEFAULT_FOLLOW,
      augment = DEFAULT_AUGMENT,
      defuse = DEFAULT_DEFUSE,
      responseBodySniffSize = DEFAULT_RESPONSE_BODY_SNIFF_SIZE,
    }
  ) {
    const results = []
    ...
    const { responseCode, responseHeaders, responseBody } =
      await this.scheduler.request({
        uri,
        headers,
        timeout,
        follow,
        downloadLimit: DEFAULT_RESPONSE_BODY_SNIFF_SIZE,
    ...
    const responseBodySniff = responseBody
      ? responseBody.slice(0, responseBodySniffSize).toString()
      : ''

    const signature = uuid.v5(
      [
        responseCode,
    ...
    if (responseBodySniff) {
      ...
      const generatorMatch = responseBodySniff.match(
        /<meta\s+name="generator"\s+content="(.+?)"|<meta\s+content="(.+?)"\s+name="generator"/i
      )
    ...
    if (!augment || !defuse) {
      if (signature) {
        results.push({
          ...
          label: signature,
          props: { signature },
          edges: [{ source, type: FINGERPRINT_TYPE }],
        })
      }
      ...
      if (responseCode) {
        results.push({
          type: CODE_TYPE,
          label: `${responseCode}/HTTP`,
          props: { code: responseCode },
          edges: [{ source, type: FINGERPRINT_TYPE }],
        })
      }
