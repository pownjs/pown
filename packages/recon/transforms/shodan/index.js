...
class ShodanTransform extends Transform {
  ...
  async handle(
    ...
    for (;;) {
      ...
      try {
        const result = await this.scheduler.tryRequest({
          uri: `https://api.shodan.io/shodan/host/search?${query}`,
          toJson: true,
        })
      ...
      matches.forEach((match) => {
        ...
        if (extractPorts) {
          ...
          results.push({
            ...
            edges: [
              reportId,
              ipId ? (ssl ? { type: TLS_TYPE, source: ipId } : ipId) : null,
            ].filter((e) => e),
          })
        }
...
const shodanSslSearch = class extends ShodanTransform {
  ...
  filterMatch(label, match) {
    ...
    const matchesExtensions = extensions.some(({ data }) =>
      regex.test(data || '')
    )
