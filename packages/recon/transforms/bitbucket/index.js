...
const bitbucketListRepos = class extends Transform {
  ...
  async handle({ id: source = '', label = '' }) {
    ...
    const { responseBody } = await this.scheduler.tryRequest({
      uri: `https://api.bitbucket.org/2.0/repositories/${search}`,
    })
    ...
    values.forEach(({ full_name: fullName = '', owner = {} }) => {
      ...
      results.push({
        id: ownerId,
        type: BITBUCKET_MEMBER_TYPE,
        label: ownerLabel,
        image: ownerAvatar.href,
        props: { username: ownerUsername, uri: ownerUri.href },
        edges: [repoId],
      })
    })
...
const bitbucketListSnippets = class extends Transform {
  ...
  async handle({ id: source = '', label = '' }) {
    ...
    const { responseBody } = await this.scheduler.tryRequest({
      uri: `https://api.bitbucket.org/2.0/snippets/${search}`,
    })
...
const bitbucketListTeamRepos = class extends Transform {
  ...
  async handle({ id: source = '', label = '' }, options) {
    ...
    const { responseBody } = await this.scheduler.tryRequest({
      uri: `https://api.bitbucket.org/2.0/teams/${search}/repositories`,
    })
...
const bitbucketListTeamMembers = class extends Transform {
  ...
  async handle({ id: source = '', label = '' }, options) {
    ...
    const { responseBody } = await this.scheduler.tryRequest({
      uri: `https://api.bitbucket.org/2.0/teams/${search}/members`,
    })
...
module.exports = {
  ...
  bitbucketListTeamRepos,
  bitbucketListTeamMembers,
}
