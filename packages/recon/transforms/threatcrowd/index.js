...
const threatcrowdDomainReport = class extends Transform {
  ...
  async handle({ id: source = '', label = '' }, options) {
    ...
    const { subdomains: domains = [], emails = [] } =
      await this.scheduler.tryRequest({
        uri: `https://www.threatcrowd.org/searchApi/v2/domain/report/?${query}`,
        toJson: true,
      })
...
const threatcrowdIpReport = class extends Transform {
  ...
  async handle({ id: source = '', label = '' }, options) {
    ...
    const { resolutions = [] } = await this.scheduler.tryRequest({
      uri: `https://www.threatcrowd.org/searchApi/v2/ip/report/?${query}`,
      toJson: true,
    })
