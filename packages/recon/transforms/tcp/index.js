...
const TLS_DATA = Buffer.from([
  16, 0x03, 0x01, 0x00, 0xa5, 0x01, 0x00, 0x00, 0xa1, 0x03, 0x03, 0x00, 0x01,
  ...
  ,
])
const RANDOM_DATA = Buffer.from(
  Array(100)
    .fill(0)
    .map(Math.random)
    .map((r) => r * 100)
...
const tcpPortScan = class extends Transform {
  ...
  async check({
    ...
    withBanners = DEFAULT_WITH_BANNERS,
    beSmart = DEFAULT_BE_SMART,
    withCertificates = DEFAULT_WITH_CERTIFICATES,
  }) {
    ...
    if (withBanners) {
      ...
      if (beSmart) {
        ...
        } else if (protocolsMap['HTTPS'].includes(port)) {
          options.data = HTTP_DATA
          options.tls = true
  ...
  async handle([{ id: source = '', label = '' }, port], options) {
    ...
    const [open, { tls }, banner, certificate] = await this.check({
      port,
      host: label,
      ...options,
    })
    ...
    if (open) {
      ...
      results.push({
        ...
        type: PORT_TYPE,
        label: portLabel,
        props: { port, protocol, services },
        edges: [tls ? { type: TLS_TYPE, source } : source],
      })

      if (banner && banner.length) {
        const bannerLabel = crypto
          .createHash('sha1')
          .update(banner)
          .digest('hex')
  ...
  async run(
    items,
    { ports = DEFAULT_PORTS, concurrency = DEFAULT_CONCURRENCY, ...options }
  ) {
    if (ports) {
      ports = ports
        .split(',')
