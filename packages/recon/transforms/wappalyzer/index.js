...
const wappalyzerProfile = class extends Transform {
  ...
  async handle(
    { id: source = '', label = '' },
    { wappalyzerKey = process.env.WAPPALYZER_KEY || 'wappalyzer.api.demo.key' }
  ) {
    ...
    const results = await this.scheduler.tryRequest({
      uri: `https://api.wappalyzer.com/lookup/v1/?${query}`,
      headers,
      toJson: true,
    })
    ...
    return applications.map((application) => {
      return {
        type: WAPPALYZER_APPLICATION_TYPE,
        label: application,
        props: { application },
        edges: [source],
      }
