...
const pwndbSearch = class extends Transform {
  ...
  async handle({ id: source = '', label = '', type = '' }) {
    ...
    switch (type) {
      case DOMAIN_TYPE:
        body = `luser=%25&domain=${encodeURIComponent(
          label
        )}&luseropr=1&domainopr=0&submitform=em`
      ...
      case EMAIL_TYPE:
        ...
        body = `luser=${encodeURIComponent(user)}&domain=${encodeURIComponent(
          domain
        )}&luseropr=1&domainopr=0&submitform=em`
    ...
    const { responseBody } = await this.scheduler.tryRequest({
      method: 'POST',
      uri: 'http://pwndb2am4tzkvold.onion.pet/',
      body,
      maxRetries: 5,
    })
    ...
    if (preMatch) {
      ...
      while ((arrayMatch = reg.exec(preMatch[1])) !== null) {
        ...
        results.push({
          id: nodeId,
          type: PWNDB_ACCOUNT_TYPE,
          label,
          props: { recordId, user, domain, password },
          edges: [source],
        })
