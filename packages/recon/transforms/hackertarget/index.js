...
const hackertargetReverseIpLookup = class extends Transform {
  ...
  static get options() {
    return {
      noiseThreshold: {
        description:
          'The number of minimum nodes before considering the result set as noise',
        type: 'number',
        default: 100,
      },
    }
  ...
  async handle(
    { id: source = '', label = '', type: sourceType = '' },
    options
  ) {
    ...
    const { responseBody } = await this.scheduler.tryRequest({
      uri: `http://api.hackertarget.com/reverseiplookup/?${query}`,
    })
    ...
    if (
      lines.length === 0 ||
      (lines.length === 1 && lines[0].indexOf(' ') >= 0)
    ) {
      return []
    }
    ...
    if (lines.length > noiseThreshold) {
      this.warn(
        `query ${label} unlikely to produce specific results due to result set length (${lines.length}) greather than noise threshold (${noiseThreshold})`
      )
...
const hackertargetOnlinePortScan = class extends Transform {
  ...
  async handle({ id: source = '', label = '' }, options) {
    ...
    const { responseBody } = await this.scheduler.tryRequest({
      uri: `https://api.hackertarget.com/nmap/?${query}`,
    })
    ...
    const lines = text
      ...
      .split('\n')
      .map((line) => line.trim().toLowerCase())
      .filter((line) => line && /^\d+\/\w+\s+open\s+/.test(line))
    ...
    return lines.map((line) => {
      ...
      return {
        type: PORT_TYPE,
        label,
        props: { port, protocol, state, services },
        edges: [source],
      }
    })
  }
