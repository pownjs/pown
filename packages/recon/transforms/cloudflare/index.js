...
const cloudflareDnsQuery = class extends Transform {
  ...
  async query(name, type, source) {
    ...
    const { Answer = [] } = await this.scheduler.tryRequest({
      uri: `https://cloudflare-dns.com/dns-query?${query}`,
      headers,
      toJson: true,
    })
    ...
    return Answer.filter(({ data }) => data).map(({ type, data }) => {
      if (type === 1) {
        ...
        return {
          type: DOMAIN_TYPE,
          label: domain,
          props: { domain, type: 'CNAME', targetDomain },
          edges: [source],
        }
      } else if (type === 2) {
        ...
        return {
          type: DOMAIN_TYPE,
          label: domain,
          props: { domain, type: 'NS', targetDomain },
          edges: [source],
        }
      } else if (type === 15) {
        ...
        return {
          type: DOMAIN_TYPE,
          label: domain,
          props: { domain, type: 'MX', targetDomain },
          edges: [source],
        }
      } else if (type === 16) {
