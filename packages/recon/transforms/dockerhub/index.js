...
const dockerhubListRepos = class extends Transform {
  ...
  async handle({ id: source = '', label = '' }, options) {
    const {
      dockerhubKey = process.env.DOCKERHUB_KEY,
      count = 100,
      pages = Infinity,
    } = options
    ...
    for (let page = 1; page <= pages; page++) {
      ...
      const { results: items = [] } = await this.scheduler.tryRequest({
        uri: `https://hub.docker.com/v2/repositories/${search}/?${query}`,
        toJson: true,
      })
      ...
      items.forEach(({ name, namespace, description }) => {
        ...
        results.push({
          type: DOCKERHUB_REPO_TYPE,
          label: fullName,
          props: { uri, fullName, description },
          edges: [source],
        })
      })
    }
