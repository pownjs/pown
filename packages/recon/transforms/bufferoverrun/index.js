...
const bufferoverrunSubdomainSearch = class extends Transform {
  ...
  async handle(
    { id: source = '', label = '' },
    { extractIps = DEFAULT_EXTRACT_IPS }
  ) {
    const needle = `.${label}`
    ...
    const { FDNS_A, RDNS } = await this.scheduler.tryRequest({
      uri: `https://dns.bufferover.run/dns?${query}`,
      toJson: true,
    })
    ...
    ;[FDNS_A || [], RDNS || []].forEach((category) => {
      category.forEach((item) => {
        ...
        results.push({
          ...
          props: {
            domain,
            ...{ previouslySeenIPv4: ptr && isIpv4(ptr) ? ptr : undefined },
          },
