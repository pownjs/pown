exports.yargs = {
  ...
  builder: (yargs) => {
    ...
    yargs.option('select', {
      ...
      type: 'string',
      describe:
        'Select graph. Nodes will be added and linked only if graph contains at least one node',
      default: '',
    })
    ...
    yargs.option('traverse', {
      ...
      type: 'string',
      describe:
        'Traverse graph. Nodes will be added and linked only if graph contains at least one node',
      default: '',
    })
  ...
  handler: async (argv) => {
    ...
    console.table(
      Object.entries(tree).map(([kind, { total, sample }]) => {
    ...
    if (summaryFile) {
        ...
        default:
          ...
          if (highlightsSortField) {
            highlightsCollection = highlightsCollection.sort((nodeA, nodeB) => {
              return (
                nodeB.data('props')[highlightsSortField] -
                nodeA.data('props')[highlightsSortField]
              ) // TODO: make it more generic by using . notation
            })
          }

          highlightsCollection = Array.from(
            new Set(highlightsCollection.map((node) => node.data('label')))
          ).slice(0, highlightsMaxSize)
          ...
          const expandItems = allItems.filter(([kind]) =>
            expandTypes.some((et) => et.includes(kind))
          )
          const filteredItems = allItems.filter(([kind]) =>
            expandTypes.every((et) => !et.includes(kind))
          )
      ...
      await fs.promises.writeFile(summaryFile, data)
    }
  },
}
