exports.yargs = {
  ...
  builder: async (yargs) => {
    ...
    Object.entries({ ...compoundTransforms, auto }).forEach(
      ([transformName, transform]) => {
        const niceTransformName = transformName.toLowerCase()

        const transformAlias = !transform.alias
          ? []
        ...
        yargs.command({
          ...
          builder: (yargs) => {
            const {
              installOutputOptions,
            } = require('../../lib/handlers/output')
            ...
            yargs.options('transform-concurrency', {
              ...
              type: 'number',
              describe:
                'Number of transform operations to execute at the same time',
              default: 100,
            })
            ...
            Object.entries(transform.options).forEach(
              ([
                optionName,
                {
                  aliases,
                ...
                yargs.option(optionName, {
                  ...
                  alias: (Array.isArray(alias) ? alias : [alias]).filter(
                    (l) => l && l.length > 1
                  ),

                  describe: describe,

                  default: defaultOptions.hasOwnProperty(optionName)
          ...
          handler: async (argv) => {
            ...
            if (transformName === 'auto') {
              ...
              gRecon.registerTransforms(
                transformInWorker
                  ? Object.assign(
                      {},
                      ...Object.entries(compoundTransforms).map(
                        ([name, transform]) => {
                          return {
              ...
              gRecon.registerTransforms({
                [transformName]: transformInWorker
                  ? wrapInWorker(transform)
                  : transform,
              })
            }
            ...
            if (select) {
              ...
              if (load) {
                ...
                const {
                  unrollOfParallel,
                } = require('@pown/async/lib/unrollOfParallel')

                realNodes = await toArray(
                  unrollOfParallel(realNodes.map(iterateFileLines))
                )
              }

              await gRecon.addNodes(
                realNodes
                  .filter((node) => node)
                  .map((node) => ({
            ...
            if (cacheMemcachedServer) {
              ...
              cache = new Cache({
                hosts: [cacheMemcachedServer],
                ttl: cacheTtl,
                keyPrefix: cacheKeyPrefix,
                keySuffix: cacheKeySuffix,
              })

              console.info(
                'caching with memcached server',
                JSON.stringify(cacheMemcachedServer)
              )
            } else if (cacheDynamodbTable) {
              const { Cache } = require('../../../../lib/cache/dynamodb')

              cache = new Cache({
                table: cacheDynamodbTable,
                ttl: cacheTtl,
                keyPrefix: cacheKeyPrefix,
                keySuffix: cacheKeySuffix,
              })
              ...
              console.info(
                'caching with dynamodb table',
                JSON.stringify(cacheDynamodbTable)
              )
            }
            ...
            try {
              await gRecon.transform(transformName, options, {
                transformConcurrency,
                nodeConcurrency,
