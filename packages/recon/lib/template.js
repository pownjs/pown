...
class ReconTemplate extends Template {
    ...
    if (['request'].includes(taskName)) {
      ...
      if (nodes) {
        if (nodes.length) {
              ...
              nodes.map(async (node) => {
                ...
                if (await this.matchWithTask(task, nodeData)) {
                  ...
                  const {
                    ...
                    label,
                    props = {},
                  } = deepmerge(extract, data)

                  return {
                    id: id ? id : `${type}:${label}`,
                    type,
                    label: this.interpolate(label, input),
                    props: this.interpolate(props, input),
                    edges: [nodeData.id],
                  }
                }
              })
          ...
          } else if (['remove'].includes(taskName)) {
            nodes = await Promise.all(
              nodes.map(async (node) => {
                const nodeData = node.data()
          ...
          } else if (['transform'].includes(taskName)) {
            nodes = await Promise.all(
              nodes.map(async (node) => {
                const nodeData = node.data()
  ...
  async *runTaskSetIt(taskName, tasks, input = {}, recon, ...args) {
    ...
    } else if (['select', 'selection'].includes(taskName)) {
      for (let task of tasks) {
        const {
          selection,
          select = selection,
    ...
    } else if (['traverse', 'traversal'].includes(taskName)) {
      for (let task of tasks) {
        const {
          traversal,
          traverse = traversal,
