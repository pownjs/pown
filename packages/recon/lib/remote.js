...
class RemoteTransform extends Transform {
  ...
  async *paginate({ id: source = '', label = '' }, options) {
    ...
    const {
      ...
      url,
      body,
      continuationId: continuationIdPath,
      continuationMethod = 'GET',
      continuationUrl,
      continuationBody,
      continuationDelay = 30000,
      items: itemsPath,
    } = transform
    ...
    for await (const items of this.paginate(node, options)) {
      const subresults = items.map((item) => {
        ...
        return {
          ...
          edges: edges
            ? edges.map((edge) =>
                typeof edge === 'string' ? edge : { source, ...edge }
              )
            : [source],
        }
      })
  ...
  async run(nodes, options, concurrency) {
    ...
    const { concurrency: definitionTransformConcurrency = concurrency } =
      transform
...
const resolveUrl = (from, to) => {
  ...
  return url
    .resolve(from, to.split('{').join(c1).split('}').join(c2))
    .split(c1)
    .join('{')
    .split(c2)
    .join('}')
...
module.exports = {
  fetchRemoteTransforms,
  buildRemoteTransforms,
  buildRemoteTransform,
}
