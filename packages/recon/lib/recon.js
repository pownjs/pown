...
class Recon extends Graph {
  ...
  registerTransforms(transforms) {
    Object.entries(transforms).forEach(([name, transform]) => {
      this.transforms[name.toLowerCase()] = transform.load
        ? transform.load()
        : transform // TODO: remove this type of loading
    })
  }
  ...
  async *itr(transformation, options = {}, settings = {}) {
    ...
    const {
      transformConcurrency = Infinity,
      nodeConcurrency = Infinity,
      timeout = 0,
      group = false,
      filter,
      extract,
      maxNodesWarn = this.maxNodesWarn,
      maxNodesCap = this.maxNodesCap,
      optionsInstance = this.optionsInstance,
      scheduler,
      cache,
    } = settings
      ...
      .filter(({ TransformClass }) => {
        ...
        if (filter) {
          ...
          if (name || alias || title || tag) {
            return (
              (name && name.test(TransformClass.name)) ||
              (alias &&
                TransformClass.alias.some((alias) => alias.test(alias))) ||
              (title && title.test(TransformClass.title)) ||
              (tag && TransformClass.tags.some((tag) => tag.test(tag)))
            )
          }
        }
    ...
    if (['*', 'auto'].includes(transformation)) {
          ...
          new Set(
            nodes.map(({ type, label }) => {
              ...
              if (label) {
                if (isUrl(label)) {
                  types.push(URL_TYPE)
                } else if (isEmail(label)) {
                  types.push(EMAIL_TYPE)
                } else if (isIpv4(label)) {
                  types.push(IPV4_TYPE)
                } else if (isIpv6(label)) {
                  types.push(IPV6_TYPE)
                } else if (isDomain(label)) {
      ...
      transforms = transforms.filter(({ constructor = {} }) =>
        constructor.types.some((type) => nodeTypes.includes(type))
      )
    }
    ...
    const handleTransform = async (transform) => {
      ...
      if (transformation === '*') {
        actualNodes = nodes.filter(({ type }) =>
          transform.constructor.types.includes(type)
        )
      } else {
        actualNodes = nodes
      }
      ...
      this.emit(
        'info',
        `starting transform ${quotedTitle} on ${actualNodes.length} nodes`
      )
      ...
      const transformCategories = !transform.constructor.category
        ? []
        : Array.isArray(transform.constructor.category)
        ? transform.constructor.category
        : [transform.constructor.category]
      const transformAliases = !transform.constructor.alias
        ? []
        : Array.isArray(transform.constructor.alias)
        ? transform.constructor.alias
        : [transform.constructor.alias]
      const transformNames = [transform.name].concat(
        transformAliases,
        transformCategories
      )
      ...
      try {
        const tasks = [
          transform.run(actualNodes, actualOptions, nodeConcurrency),
        ]
      ...
      if (results && results.length) {
        this.emit(
          'info',
          `transform ${quotedTitle} finished with ${results.length} results`
        )
        ...
        if (maxNodesWarn && results.length > maxNodesWarn) {
          this.emit(
            'warn',
            `transform ${quotedTitle} will add ${results.length} nodes`
          )
        }
        ...
        if (maxNodesCap && results.length > maxNodesCap) {
          this.emit(
            'warn',
            `transform ${quotedTitle} nodes capped to ${maxNodesCap}`
          )
    ...
    for await (let results of iterateOfLimit(
      transforms,
