...
const listWebArchiveURIs = async function* (domain, options) {
  ...
  const pageTotal = await requestJSON(scheduler, {
    ...schedulerOptions,
    uri: `http://web.archive.org/cdx/search/cdx?${search}&${preQuery}&showNumPages=true`,
  })
  ...
  for (let { page, status } of generatePages()) {
    ...
    const data = await requestJSON(scheduler, {
      ...schedulerOptions,
      uri: `http://web.archive.org/cdx/search/cdx?${search}&${preQuery}&${postQuery}`,
    })
