...
const { ensureArray, ensureObject, btoa, atob, dict, list, ret } = require('./util.js')
...
class Template {
    ...
    async test(matcher, input) {
        if (this.isString(matcher)) {
            ...
            const { part, condition, lt, lessthan, gt, greaterthan, lte, lessthanorequal, gte, greaterthanorequal, eq, equal, number, string, word, words, regex, flag, flags, b64, base64, entropy, script } = /** @type {DetailedMatcher} */(matcher)
            ...
            if (!type) {
                ...
                if (lte || lessthanorequal) {
                    type = 'lessthanorequal'
                }
                else
                if (gte || greaterthanorequal) {
            ...
            switch (type) {
                case 'lt':
                case 'lessthan':
                    return this.getTestConditionFunc(condition)(ensureArray(eq || equal || number || NaN), (number) => {
                ...
                case 'words':
                    if (this.isString(input) || this.isBuffer(input)) {
                        return this.getTestConditionFunc(condition)(ensureArray(word || words || ''), (word) => {
                ...
                case 'regex':
                    if (this.isString(input) || this.isBuffer(input)) {
                        return this.getTestConditionFunc(condition)(ensureArray(regex || ''), (regex) => {
                ...
                case 'base64':
                    if (this.isString(input)) {
                        return this.getTestConditionFunc(condition)(ensureArray(b64 || base64 || ''), (b64) => {
                ...
                case 'script':
                    return this.getTestConditionFunc(condition)(ensureArray(script || ''), (script) => {
    ...
    async match(matcher, input) {
        if (this.isString(matcher)) {
            ...
            const { negative, not, ...rest } = /** @type {DetailedMatcher} */(matcher)
    ...
    async extract(extractor, input) {
        if (this.isString(extractor)) {
            ...
            const { part, name, value, jsonpath, path, regex, flag, flags, group, script } = /** @type {DetailedExtractor} */(extractor)
    ...
    async runTask(taskName, task, input, ...args) {
        ...
        try {
            const taskResult = await this.executeTask(taskName, await this.getTaskDefinition(task), input, ...args)
        ...
        if (error) {
            ...
            return { id: task.id, name: taskName, result: null, error, input, matches: false, extracts: null, output: null }
        }
        else {
            this.tracer.push(`Task execution finished`, { name: taskName, task, input, result })
            ...
            return { id: task.id, name: taskName, result, error, input, matches, extracts, output }
        }
    }
    ...
    async * runIt(input = {}, ...args) {
        ...
        this.tracer.push(`Executing template`, { id, template: this.template, input })

        yield* this.runTaskDefinitionsIt(tasks, input, ...args)

        const finishedAt = Date.now() // TODO: get nanosecond precision

        this.tracer.push(`Template execution finished`, { id, template: this.template, input, startedAt, finishedAt })
    }
