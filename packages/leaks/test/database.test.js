...
describe('database', () => {
  ...
  const database = compileDatabase(
    Object.assign(
      {},
      ...fs.readdirSync(databaseRoot).map((file) => {
        const doc = jsYaml.load(
          fs.readFileSync(path.join(databaseRoot, file)).toString()
        )
        ...
        for (let check of checks) {
          Object.entries(variables).forEach(([name, value]) => {
            ...
            if (check.filterRegex) {
              check.filterRegex = check.filterRegex
                .split('${' + name + '}')
                .join(value)
            }
          })
  ...
  it('database checks validate', () => {
    ...
    Object.entries(database).forEach(
      ([name, { variables = {}, checks = [] }]) => {
        checks.forEach((check) => {
          ...
          assert.ok(
            !test,
            `${JSON.stringify(title)} has incorrect test declaration`
          )
            ...
            if (positive) {
              ...
              positive.forEach((test, index) => {
                ...
                assert.ok(
                  ...
                  `${JSON.stringify(
                    title
                  )} validates against positive test ${JSON.stringify(
                    test
                  )} at index ${index}`
                )
              })
            ...
            if (negative) {
              ...
              negative.forEach((test, index) => {
                ...
                assert.ok(
                  ...
                  `${JSON.stringify(
                    title
                  )} validates against negative test ${JSON.stringify(
                    test
                  )} at index ${index}`
                )
              })
            ...
            if (safe) {
              if (regex) {
                assert.ok(
                  ...
                  `${JSON.stringify(
                    title
                  )} validates safe regex test ${regex.toString()}`
                )
              }

              if (filterRegex) {
                assert.ok(
                  safeRegex(filterRegex),
                  `${JSON.stringify(
                    title
                  )} validates safe filter regex test ${regex.toString()}`
                )
              }
            }
          }
