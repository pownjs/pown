...
describe('dataset', () => {
  it('viv-1615539741', async () => {
    const dataset = jsYaml.load(
      fs.readFileSync(
        path.join(__dirname, '..', 'dataset', 'viv-1615539741.yaml')
      )
    )
    ...
    for (let { valid, invalid } of dataset) {
      if (valid) {
        ...
        assert.ok(
          lastMatch,
          `Valid secret ${JSON.stringify(valid)} should match`
        )
      } else {
        let lastMatch
        ...
        assert.ok(
          !lastMatch,
          `Invalid secret ${JSON.stringify(
            invalid
          )} should not match ${JSON.stringify(lastMatch, '', '  ')}`
        )
      }
    }
  })
