...
const extractPownModules = callbackify(memorize('extractPownModules', async(paths) => {
    ...
    modules.forEach((module) => {
        ...
        if (module.config.transform) {
            loadableTransforms = loadableTransforms.concat([path.resolve(module.realpath, module.config.transform)])
        }

        if (module.config.transforms) {
            loadableTransforms = loadableTransforms.concat(module.config.transforms.map(transform => path.resolve(module.realpath, transform)))
        }
