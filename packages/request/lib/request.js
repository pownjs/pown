...
const maybeUnzip = async(buf, headers) => {
    try {
        ...
        switch (contentEncoding) {
            case 'gzip':
                return await gunzipAsync(buf, { finishFlush: zlib.constants.Z_FULL_FLUSH })

            case 'deflate':
                return await deflateAsync(buf, { finishFlush: zlib.constants.Z_FULL_FLUSH })

            case 'br':
                return await brotliDecompressAsync(buf, { finishFlush: zlib.constants.Z_FULL_FLUSH })
        }
...
const requestInternal = (request, resolve, followCount = 0) => {
    const { type = 'base', method = 'GET', uri, version = 'HTTP/1.1', headers: _headers = EMPTY_OBJECT, body, info, timeout = 30000, connectTimeout = timeout, dataTimeout = timeout, follow = false, followLimit = 10, download = true, downloadLimit = Infinity, certificate = false, correctHeaders = true, rejectUnauthorized = true, client, ...rest } = request
